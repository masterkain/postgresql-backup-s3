...
def check_environment_variables(required_vars):
    ...
    for var in required_vars:
        if not os.getenv(var):
            ...
            if (
                "PASSWORD" not in var.upper()
                and "KEY" not in var.upper()
                and "SECRET" not in var.upper()
            ):
    ...
    if version_output:
        try:
            ...
            if major_version.isdigit():
                ...
                logging.error(
                    f"Could not parse major version number from output: '{version_output}'"
                )
                return None
        except IndexError:
            logging.error(
                f"Unexpected format for PostgreSQL version output: '{version_output}'"
            )
            return None
    else:
...
def list_databases(postgres_opts):
    ...
    if specific_db:
        ...
        if databases:
            ...
            logging.warning(
                "No user databases found to back up (excluding postgres, template0, template1)."
            )
        return databases
    ...
    try:
        ...
        if os.path.exists(dest_file) and os.path.getsize(dest_file) > 0:
            try:
                ...
                logging.error(
                    f"Dump command produced an unreadable gzip file '{dest_file}': {gzip_error}"
                )
                os.remove(dest_file)
                return None
            ...
            if not first_byte:
                logging.error(
                    f"Dump command produced an empty gzip payload for '{db_name}'."
                )
                os.remove(dest_file)
                return None
            ...
            logging.info(
                f"Database '{db_name}' dumped successfully to '{dest_file}' ({file_size} bytes)."
            )
            return dest_file
        else:
            ...
            logging.error(
                f"Dump command ran but '{dest_file}' is missing or empty. Stderr: {stderr_info}"
            )
        ...
        stderr_stripped = e.stderr.strip() if e.stderr else "No stderr output"
        stdout_stripped = (
            e.stdout.strip() if e.stdout else "No stdout output"
        )  # gzip might output to stdout on error
        ...
        if os.path.exists(dest_file):
            ...
            try:
                ...
                logging.error(
                    f"Error removing incomplete dump file '{dest_file}': {remove_err}"
                )
        return None
    except Exception as e:
...
def encrypt_dump(src_file, password):
    ...
    if run_command(command, sensitive=True) is not None:
        try:
            ...
            logging.info(
                f"Encryption successful. Encrypted file: {enc_file}. Original removed: {src_file}"
            )
            return enc_file
        except OSError as e:
            logging.error(
                f"Encryption seemed successful, but failed to remove original file '{src_file}': {e}"
            )
        ...
        if os.path.exists(enc_file):
            ...
            try:
                ...
                logging.error(
                    f"Error removing incomplete encrypted file '{enc_file}': {remove_err}"
                )
        return None
...
def cleanup_old_backups(
    bucket, prefix, older_than_str, active_databases, endpoint_option=""
):
    """Removes backups older than the specified period from S3."""
    logging.info(
        f"Starting cleanup of backups older than '{older_than_str}' in s3://{bucket}/{prefix}/"
    )
    ...
    try:
        ...
        if (
            ...
            or parts[1].lower() not in ["day", "days"]
        ):
        ...
        logging.info(
            f"Cleanup cutoff date (UTC): {cutoff_date.strftime('%Y-%m-%d %H:%M:%S %Z')}"
        )
    except (ValueError, IndexError):
        logging.error(
            f"Invalid format for DELETE_OLDER_THAN: '{older_than_str}'. Expected format like '30 days'. Aborting cleanup."
        )
        return
    ...
    for line in lines:
        ...
        try:
            ...
            logging.error(
                f"Could not parse date/time '{last_modified_str}' from S3 listing for file '{file_name}': {ve}"
            )
            error_count += 1
            continue
        ...
        if not match:
            logging.warning(
                f"Skipping file '{file_name}': does not match expected naming pattern 'dbname_YYYY-MM-DDTHHMMSSZ.sql.gz[.enc]'."
            )
        ...
        if db_name not in active_databases:
            logging.info(
                f"Skipping deletion of '{file_name}': Extracted database name '{db_name}' is not in the current active list: {active_databases}"
            )
            skipped_db_count += 1
            continue
        ...
        if last_modified_date < cutoff_date:
            delete_command = (
                f"aws s3 rm {endpoint_option} s3://{bucket}/{prefix}/{file_name}"
            )
            logging.info(
                f"Deleting '{file_name}' (Last Modified: {last_modified_date.strftime('%Y-%m-%d %H:%M:%S %Z')}, older than cutoff)"
            )
            ...
            logging.debug(
                f"Keeping '{file_name}' (Last Modified: {last_modified_date.strftime('%Y-%m-%d %H:%M:%S %Z')}) as it is not older than cutoff"
            )
            kept_count += 1

    logging.info(f"Cleanup finished for s3://{bucket}/{prefix}/.")
    logging.info(
        f"Summary: Deleted={deleted_count}, Kept={kept_count}, Skipped (Inactive DB)={skipped_db_count}, Skipped (Pattern Mismatch)={skipped_pattern_count}, Errors={error_count}"
    )
...
def main():
    ...
    if aws_region:
        ...
        logging.warning(
            "S3_REGION not set, defaulting to us-east-1. Consider setting S3_REGION."
        )
    ...
    original_prefix = os.getenv("S3_PREFIX", "").strip(
        "/"
    )  # Remove leading/trailing slashes
    # Combine original prefix and version prefix correctly
    full_prefix_parts = [
        p for p in [original_prefix, version_prefix] if p
    ...
    if not databases:
        logging.warning(
            "No databases found or specified to back up. Skipping dump and upload steps."
        )
    else:
        ...
        if encryption_password:
            ...
            logging.info(
                "ENCRYPTION_PASSWORD is not set. Backups will not be encrypted."
            )
        ...
        for db in databases:
            ...
            if not dumped_file_path:
                logging.error(
                    f"Dump failed for database '{db}'. Skipping upload and encryption."
                )
                failed_dumps += 1
            ...
            if encryption_password:
                encrypted_file_path = encrypt_dump(
                    dumped_file_path, encryption_password
                )
            ...
            s3_filename = os.path.basename(
                file_to_upload
            )  # e.g., db_ts.sql.gz or db_ts.sql.gz.enc
    ...
    if delete_older_than:
        if not databases:
            logging.warning(
                "Skipping cleanup because no active databases were determined in this run."
            )
        else:
            # Pass the list of databases found *in this run* to the cleanup function
            cleanup_old_backups(
                bucket, full_prefix, delete_older_than, databases, endpoint_option
            )
    else:
