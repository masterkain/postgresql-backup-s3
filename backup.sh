...
if [ "${DELETE_OLDER_THAN}" != "**None**" ]; then
  echo >&2 "Checking for files older than ${DELETE_OLDER_THAN}"
  aws $AWS_ARGS s3 ls s3://$S3_BUCKET/$S3_PREFIX/ | grep " PRE " -v | while read -r line; do
    ...
    if [ $created -lt $older_than ]; then
      if [ $fileName != "" ]; then
        echo >&2 "DELETING ${fileName}"
        aws $AWS_ARGS s3 rm s3://$S3_BUCKET/$S3_PREFIX/$fileName
      fi
    else
      echo >&2 "${fileName} not older than ${DELETE_OLDER_THAN}"
    fi
  done
fi
